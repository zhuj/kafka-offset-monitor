...
import com.morningstar.kafka._
        ...
        while (!hasError) {
          lazy val f = Future {
            try {
              ...
              val groupOverviews = adminClient.listAllConsumerGroups()
              groupOverviews.values.flatten.foreach(groupOverview => {
